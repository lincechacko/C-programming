...
if(get_image_size_for_bmp(encInfo->fptr_src_image)>(54+16+32+(sizeof(encInfo->extn_secret_file)*8)+32+(secret_file1*8)))
{
	return e_success;
}
}
...
Status encode_secret_file_extn_size(int size,FILE *src_image,FILE *dest_image)
{
	char str[32];
...
if(encode_data_to_image(str1,size1,encInfo->fptr_src_image,encInfo->fptr_stego_image,encInfo)==e_success)
{
	return e_success;
}
}

Status copy_remaining_img_data(FILE *src_image,FILE *dest_image)
{
char ch;
